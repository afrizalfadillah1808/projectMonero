...
@section('content')
  <div class="m-0 col-auto col-md-9 col-xl-10 px-sm-2 pt-5">
    ...
    <form action="/dashboard/discounts/{{ $discount->kodeDiscount }}" method="POST" class="ms-5 col-md-5 pt-2" enctype="multipart/form-data">
      ...
      <div class="mb-3">
        <a href="/dashboard/discounts" class="bg-indigo-600 px-4 py-2 fw-medium shadow-sm rounded-xl text-center text-indigo-100 text-decoration-none"><i class="fa-solid fa-arrow-left text-indigo-100 me-1"></i> Back</a>
        <button type="submit" class="bg-indigo-600 px-4 py-2 fw-medium shadow-sm rounded-xl text-center text-indigo-100 text-decoration-none border-0" style="text-decoration: none;">Edit Discount</button>
      </div>
    </form>
  </div>
@endsection