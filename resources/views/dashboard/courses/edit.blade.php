...
@section('content')
  <div class="m-0 col-auto col-md-9 col-xl-10 px-sm-2 pt-5">
    ...
    <form action="/dashboard/courses/{{ $course->slugCourse }}" method="POST" class="ms-5 col-md-5 pt-2" enctype="multipart/form-data">
      ...
      <div class="mb-3">
        <input type="file" class="@error('imgCourse') is-invalid @enderror border-0 bg-indigo-115 rounded-2 text-capitalize p-2 w-100 text-indigo-900" id="imgCourse" name="imgCourse" placeholder="Course Thumbnail" autofocus required>
        @error('imgCourse')
          <div class="invalid-feedback text-capitalize">
            {{ $message }}
          </div>
